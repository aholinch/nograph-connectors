...
/**
 * Store nodes, rels, and associated properties in a JDBC-accessible database.
 * 
 * @author aholinch
 *
 */
public class JdbcGraphManager extends BaseGraphManager 
{
    protected DBConnection dbConn;
    
    public JdbcGraphManager()
    {
    	init();
    }
    
    public JdbcGraphManager(NoGraphConfig config)
    {
    	init(config);
    }
    ...
    protected void init()
    {
    	init(new NoGraphConfig());
    }
    
    protected void init(NoGraphConfig config)
    {
	 ...
	 *
	 */
	protected static class DBConnection
	{
		/**
		 * A Jodd utility that we will wrap.
		 */
		protected ConnectionProvider connProvider;
		
		public DBConnection(NoGraphConfig config)
		{
			initProvider(config);
		}
		
		protected void initProvider(NoGraphConfig config)
		{
			CoreConnectionPool connPool = new CoreConnectionPool();
			...
			connPool.setDriver(driver);
			connPool.setUrl(url);
			connPool.setMinConnections(min);
			connPool.setMaxConnections(max);
			...
			connProvider = connPool;
		}
		
		/**
		 * May be a new connection or from a connection cache.
		 * 
		 * @return
		 */
		...
		{
			if(stmt != null)try{stmt.close();}catch(Exception ex){}; // quiet
		}
		
		public static void close(ResultSet rs)
		{
			if(rs != null)try{rs.close();}catch(Exception ex){}; // quiet			
		}
	}
